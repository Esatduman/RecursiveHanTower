...
int HanoiRecursiveN(int n)	// recursive version
{
	...
	{
		return 2 * HanoiRecursiveN(n - 1) + 1;
	}
}
...
void HanoiRecursiveSteps(size_t n, string start, string temp, string target)
{
	if (n >= 1)
	{
	    HanoiRecursiveSteps(n - 1, start, target, temp);
	    cout << "Disk_" << n << ": " << start << " --> " << target << endl;
	    HanoiRecursiveSteps(n - 1, temp, start, target);
	}
}
